...
{
    ERHook::ERHook(std::shared_ptr<ManagedProcess> process)
    : BaseHook(std::move(process))
    {
        ERHook::RefreshAllPointers();
    }

    void ERHook::RefreshAllPointers()
    {
    ...
    uint32_t ERHook::GetNGPlusLevel() const
    {
        return m_GameDataMan->Read<uint32_t>(GAME_DATA_MAN::NG_PLUS_LEVEL);
    }

    void ERHook::SetNGPlusLevel(uint32_t level) const
    {
        if (!m_GameDataMan->Write<uint32_t>(GAME_DATA_MAN::NG_PLUS_LEVEL, level))
            Error(std::format("Failed to set NG+ level to {}.", level));
    }
}

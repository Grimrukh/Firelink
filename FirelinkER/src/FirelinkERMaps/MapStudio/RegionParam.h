...
{
    class FIRELINK_ER_MAPS_API RegionParam final : public EntryParam<Region>
    {
    public:
        RegionParam()
        : EntryParam(73, u"POINT_PARAM_ST")
        {
        }
        ...
        [[nodiscard]] Region* GetNewEntry(const int entrySubtype) override
        {
            ...
            switch (entrySubtype)
            {
                ...
                default:
                    throw MSBFormatError(std::format("Invalid Region subtype: {}", static_cast<int>(entrySubtype)));
            }

...
namespace FirelinkER::Maps::MapStudio
{
    ...
    template <typename T>
    class FIRELINK_ER_MAPS_API EntryReference final : public EntryReferenceBase
    {
    public:
        ...
        explicit EntryReference(T* destEntry)
    ...
    void SetReferenceArray16(
        std::array<EntryReference<T>, N>& references, std::array<int16_t, N> indices, const std::vector<T*>& entries)
    {
        for (size_t i = 0; i < N; i++)

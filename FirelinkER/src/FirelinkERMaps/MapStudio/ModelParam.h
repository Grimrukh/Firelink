...
namespace FirelinkER::Maps::MapStudio
{
    ...
    public:
        ModelParam()
        : EntryParam(73, u"MODEL_PARAM_ST")
        {
        }
        ...
        [[nodiscard]] Model* GetNewEntry(const int entrySubtype) override
        {
            ...
            switch (entrySubtype)
            {
                case static_cast<int>(ModelType::MapPiece):
                {
                    newModel = std::make_unique<MapPieceModel>();
                    break;
                }
                case static_cast<int>(ModelType::Character):
                {
                    newModel = std::make_unique<CharacterModel>();
                    break;
                }
                case static_cast<int>(ModelType::Player):
                {
                    newModel = std::make_unique<PlayerModel>();
                    break;
                }
                case static_cast<int>(ModelType::Collision):
                {
                    newModel = std::make_unique<CollisionModel>();
                    break;
                }
                case static_cast<int>(ModelType::Asset):
                {
                    newModel = std::make_unique<AssetModel>();
                    break;
                }
                default:
                    throw MSBFormatError(std::format("Invalid Model subtype: {}", static_cast<int>(entrySubtype)));
            }

...
{
    class FIRELINK_ER_MAPS_API EventParam final : public EntryParam<Event>
    {
    public:
        EventParam()
        : EntryParam(73, u"EVENT_PARAM_ST")
        {
        }
        ...
        [[nodiscard]] Event* GetNewEntry(const int entrySubtype) override
        {
            ...
            switch (entrySubtype)
            {
                ...
                default:
                    throw MSBFormatError(std::format("Invalid Event subtype: {}", static_cast<int>(entrySubtype)));
            }

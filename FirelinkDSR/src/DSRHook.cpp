...
DSRHook::DSRHook(shared_ptr<ManagedProcess> process) : BaseHook(std::move(process))
{
...
void DSRHook::RefreshChildPointers()
{
    ...
    m_PlayerGameData = m_process->CreateChildPointer(
        "PlayerIns",
        "PlayerGameData",
        { PLAYER_INS::PLAYER_GAME_DATA_PTR });

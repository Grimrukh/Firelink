...
namespace GrimHook::BinaryReadWrite
{
    ...
    GRIMHOOK_API void AssertReadPadBytes(std::ifstream& stream, const int& padSize, const std::string& fieldName, const uint8_t& padValue = 0);
    ...
    GRIMHOOK_API void WritePadBytes(std::ofstream& stream, const int& padSize, const uint8_t& padValue = 0);
    ...
    GRIMHOOK_API std::string ReadString(std::ifstream& stream);
    /// @brief Reads a null-terminated UTF-8 string from the stream at a specific `offset`.
    GRIMHOOK_API std::string ReadString(std::ifstream& stream, const std::streampos& offset);

    /// @brief Writes a null-terminated UTF-8 string to the stream.
    GRIMHOOK_API void WriteString(std::ofstream& stream, const std::string& str);
    /// @brief Writes a null-terminated UTF-8 string to the stream at a specific `offset`.
    GRIMHOOK_API void WriteString(std::ofstream& stream, const std::streampos& offset, const std::string& str);

    /// @brief Reads a null-terminated UTF-16 string from the stream.
    GRIMHOOK_API std::u16string ReadUTF16String(std::ifstream& stream);
    /// @brief Reads a null-terminated UTF-16 string from the stream at a specific `offset`.
    GRIMHOOK_API std::u16string ReadUTF16String(std::ifstream& stream, const std::streampos& offset);

    /// @brief Writes a null-terminated UTF-16 string to the stream.
    GRIMHOOK_API void WriteUTF16String(std::ofstream& stream, const std::u16string& str);
    /// @brief Writes a null-terminated UTF-16 string to the stream at a specific `offset`.
    GRIMHOOK_API void WriteUTF16String(std::ofstream& stream, const std::streampos& offset, const std::u16string& str);

    /// @brief Writes a null-terminated UTF-16 string to the stream (converted from UTF-8)
    GRIMHOOK_API void WriteUTF16String(std::ofstream& stream, const std::string& str);
    /// @brief Writes a null-terminated UTF-16 string to the stream at a specific `offset` (converted from UTF-8)
    GRIMHOOK_API void WriteUTF16String(std::ofstream& stream, const std::streampos& offset, const std::string& str);
    ...
    GRIMHOOK_API void AlignStream(std::ifstream& stream, const std::streamsize& alignment);
    GRIMHOOK_API void AlignStream(std::ofstream& stream, const std::streamsize& alignment);

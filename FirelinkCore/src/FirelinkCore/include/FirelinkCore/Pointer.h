...
namespace Firelink
{
    ...
    /// address.
    class FIRELINK_CORE_API BasePointer
    {
    public:
    ...
    protected:
        ...
        BasePointer(
            const ManagedProcess& process, const void* address, std::string name, const std::vector<int>& offsets = {})
            : m_process(process)
        ...
        friend BasePointer* ManagedProcess::CreatePointerWithJumpInstruction(
            const std::string& name, const void* baseAddress, int jumpRelativeOffset, const std::vector<int>& offsets);
        ...
        friend BasePointer ManagedProcess::CreateTempPointer(const std::string& name, const void* baseAddress) const;
    };

    /// @brief EXPORT: Child pointer class whose base address is taken from the resolved address of a parent pointer.
    class FIRELINK_CORE_API ChildPointer final : public BasePointer
    {
    public:

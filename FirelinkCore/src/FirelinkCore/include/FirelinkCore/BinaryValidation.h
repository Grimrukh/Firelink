...
namespace Firelink::BinaryValidation
{
    ...
    void ValidateSinglePadding(const std::string& typeName, const padding<Size>& pad)
    {
        ...
        {
            throw BinaryReadWrite::BinaryAssertionError(typeName + ": Non-zero padding field detected.");
        }
    }
    ...
    void AssertValue(const std::string& typeName, const R& reference, const T& value)
    {
        if (reference != value)
        {
            throw BinaryReadWrite::BinaryAssertionError(
                typeName + ": Value validation failed (expected " + std::to_string(reference) + ", got "
                + std::to_string(value) + ")");
        }
    }
    ...
    void AssertNotValue(const std::string& typeName, const R& reference, const T& value)
    {
        if (reference == value)
            throw BinaryReadWrite::BinaryAssertionError(
                typeName + ": Value validation failed (expected NOT " + std::to_string(reference) + ")");
    }

    // Function template to validate that given array value is equal to a reference array.
    template <std::integral T, std::size_t N>
    void ValidateArray(
        const std::string& fieldName, const std::array<T, N>& reference, const std::array<T, N>& arrayValue)
    {

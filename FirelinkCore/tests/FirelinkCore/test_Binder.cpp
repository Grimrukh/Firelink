...
namespace
{
    ...
    std::optional<Binder> load_binder_dcx(const char* name)
    {
        auto path = find_resource(name);
...
{
    auto binder = load_binder_dcx("c2300.chrbnd.dcx");
    if (!binder.has_value())
    {
...
{
    auto binder = load_binder_dcx("c2010.anibnd.dcx");
    if (!binder.has_value())
    {
...
namespace
{
    ...
    {
        auto bhd_path = find_resource(bhd_name);
        auto bdt_path = find_resource(bdt_name);
        auto bhd_raw = load_file(bhd_path);
        auto bdt_raw = load_file(bdt_path);
...
{
    auto binder = load_split_binder("c2300.chrtpfbhd", "c2300.chrtpfbdt");
    if (!binder.has_value())
    {

...
//
// Converts every .dds fixture in resources/dds/ to both TGA and PNG
...
namespace
{
    ...
    std::filesystem::path dds_resource_dir()
    {
        return std::filesystem::path(TEST_RESOURCES_DIR) / "dds";
    }
    ...
    bool looks_like_tga(const std::vector<std::byte>& data)
    {
    ...
    bool looks_like_png(const std::vector<std::byte>& data)
    {
    ...
    bool looks_like_dds(const std::vector<std::byte>& data)
    {
...
TEST_CASE("DDS: ConvertDDSToTGA converts every fixture")
{
    ...
    if (files.empty())
    {
        MESSAGE("Skipping — no .dds fixtures found in resources/dds/");
        return;
    }
    ...
    for (const auto& path : files)
    {
        ...
        Firelink::Info("Loading DDS: " + path.string());
        auto dds = load_file(path);
        REQUIRE(!dds.empty());

        Firelink::Info("Converting DDS to TGA: " + path.string());
        auto tga = ConvertDDSToTGA(dds.data(), dds.size());
        CHECK(!tga.empty());
        CHECK(looks_like_tga(tga));
    }
}
...
TEST_CASE("DDS: round-trip DDS -> TGA -> DDS produces valid DDS")
{
    ...
    for (const auto& path : files)
    {
        ...
        CHECK(looks_like_dds(roundtrip));
    }
}

...
namespace Firelink
{
    ...
    public:
        ...
        ///
        /// @returns Actual path written, which may have a '.dcx' suffix added for compression.
        std::filesystem::path WriteToPath(std::filesystem::path path = std::filesystem::path{}) const // NOLINT(*-use-nodiscard)
        {
            if (path.empty())
                path = m_path;

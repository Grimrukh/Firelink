...
namespace Firelink
{
     ...
     */
    class FIRELINK_CORE_API ManagedProcess
    {
    public:
        ...
        [[nodiscard]] bool GetAllowPointerOverwrite() const { return m_allowPointerOverwrite; }

        /// @brief Set pointer overwrite to true or false.
        void SetAllowPointerOverwrite(const bool allow) { m_allowPointerOverwrite = allow; }
        ...
        BasePointer CreateTempPointer(const std::string& name, const void* baseAddress) const;
         ...
         **/
        [[nodiscard]] const void* FindPattern(
            const BYTE* pattern,
            SIZE_T patternSize,
            const bool* wildcardMask = nullptr,
            bool pageExecuteOnly = true) const;
        ...
        static std::unique_ptr<ManagedProcess> WaitForProcess(
            const std::wstring& processName, int timeoutMs, int refreshIntervalMs, const std::atomic<bool>& stopFlag);
    ...
    private:
        ...
        bool m_allowPointerOverwrite = true;
